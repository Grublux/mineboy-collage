...
// BlockCard component for displaying blocks
function BlockCard({
  gridId,
}: {
  gridId: bigint;
      ...
      {metadata?.image && (
        <Link href={`/collage/${gridId.toString()}`}>
          ...
          >
            <img
              src={metadata.image}
              alt={metadata.name || `Block #${gridId}`}
              style={{
                width: "100%",
                height: "100%",
                objectFit: "contain",
...
export default function MyBlocksPage() {
  ...
  if (!isConnected) {
    return (
      ...
      >
        <WalletHeader />
        <Header title="My Blocks" />
        <div style={{ textAlign: "center", padding: "40px" }}>
          Please connect your wallet to view your blocks.
        </div>
      </div>
    );
  }
    ...
    >
      <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
        ...
        {loadingGrids ? (
          ...
          >
            <p style={{ fontSize: "18px", marginBottom: "10px" }}>No blocks yet</p>
            <p style={{ fontSize: "14px" }}>
              Create your first block using one of the collection pages!
            </p>
          </div>
        ) : (
          <div
